    ...
    public:
        ...
        bool BinarySearch(int no)
        {
            ...
            while(iStart <= iEnd)
            {
                ...
                if(Arr[iMid] == no)         // improved TC in next improved funtn
                {
                    bFlag = true;
                    break;
                }
...
int main()
{
    // ArrayX aobj(5);      // static memory allocation soo use .
    int iLength = 0;
    int iValue = 0;
    bool bRet = false;
    ...
    ArrayX *aobj = new ArrayX(iLength); // Dynamic Memory Allocation soo use ->

    // aobj.Accept();   // use . in case of static mem alloc
    // aobj.Display();  
