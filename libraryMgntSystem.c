...
void saveUsersToFile() {
    FILE *file = fopen("users.dat", "wb");// Open file in binary write mode, data written in binary format
    if (!file) {
        perror("Error opening file for saving users");
        return;
    }
    fwrite(&userCount, sizeof(int), 1, file);//sizeof(int) is the size of each element, 1 is the number of elements to write, &userCount is the address of the variable to write
    fwrite(users, sizeof(User), userCount, file); //sizeof(User) is the size of each element, userCount is the number of elements to write, users is the address of the array to write
    fclose(file);
}

// Load users from a file
void loadUsersFromFile() { //here we are reading the users from a file, reading the number of users first, then reading the users themselves
    //loading users means
    ...
    while (1) {
        ...
        switch (choice) {
            case 1:
                ...
                author[strcspn(author, "\n")] = '\0'; //what does strcspn do? Remove trailing newline
